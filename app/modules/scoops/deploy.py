...
NO se ocupa de Ingress/Certificate/DNS: eso es responsabilidad de
`DomainService` (recurso `Domain` asociado al scoop).
"""
import logging
...
class DeployService:
            ...
            try:
                ...
                if K8sService.exists(kind, ns, name):
                    ...
                    try:
                        ...
                        # 409 en el create: una carrera (p.ej. un operator externo
                        # creando su propio recurso con el mismo nombre) gano al
                        # check de exists(). Tratar el recurso como existente y
                        # parchear.
                        if exc.status != 409:
                            raise
                        K8sService.replace(kind, ns, name, manifest, dry_run=dry_run)
    ...
    def undeploy(scoop: Scoop, namespace: str | None = None) -> dict:
        ...
        AuditService.log(
            ...
            {"namespace": ns, "resources": results},
        )
        return {
            "namespace": ns,
            "resources": results,
